...
uses
    gestionnaire,GestionEcran,uniteAraign;


procedure mine(var personnage : heros); //procédure ouvrant différentes possibilités au joueur
function recoltFer(personnage : heros): Integer; //fonction générant le fer récoltée s'il en trouve
...
procedure affichageStat(var personnage : heros ; var monstre : monster); //procédure permettant d'afficher en combat les statistiques du joueur et du monstre
...
begin
  ...
  readln;
    ...
    writeln('Stock de pierre : ',personnage.pierre); //affiche le stock du pierre
		...
		writeln('Vous entrez dans une salle avec plusieurs chemins. Que voulez-vous faire?'); //affiche les choix proposés pour le joueur
		writeln('1 - Recolter de la pierre');
		writeln('2 - Essayer de trouver du fer');
		writeln('3 - Tourner a gauche');
    writeln('4 - Tourner a droite');
		writeln('5 - Sortir de la mine');
		...
		case choix of
			...
			2 : personnage.fer := personnage.fer + recoltFer (personnage); //récolter du fer
			3 : cbt(personnage,monstre); //choix de lancer un combat
      4 : donjAraign (personnage, monstre);
      5 : quitter := false; //on sort de la mine
		end;
...
var
	...
	fer : integer; //variable correspondant à la quantité de fer trouvée
...
begin
  ...
  begin
    randomize;
	  ...
	  begin   //le joueur trouve du fer
		  randomize;
		  fer := (random(2)+4); //génération du fer trouvé
      writeln('Vous avez trouve ',fer,' morceaux de fer');
	  end
	  else
	  begin  //il n'en trouve pas
		  writeln('Vous n''avez pas trouve de fer');
		  fer := 0; //on renvoie 0 fer
	  end;
    readln;
  end
  else
  begin
    randomize;
	  ...
	  begin   //le joueur trouve du fer
		  randomize;
		  fer := (random(2)+1); //génération du fer trouvé
      writeln('Vous avez trouve ',fer,' morceaux de fer');
	  end
	  else
	  begin  //il n'en trouve pas
...
begin
  ...
  monstre.arme := 1;
    ...
    personnage.fer := personnage.fer + 100; //il reçoit du fer
	end
	else
	begin
